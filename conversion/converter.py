...
def convert_units(value, from_unit, to_unit, **args):
    ...
    try:
        ans = str(round(convert(from_unit, to_unit, float(value), **args), decimal_places)) #+ " " + get_si(to_unit)
        if ans[:1] == "-":
            return "minus " + num2words(float(ans[1:]), lang="en_IN")
        else:
            return num2words(ans, lang="en_IN")
